...
// Do not apply a date filter when the diary first opens.
// The timeline itself only shows dates that actually contain
// one or more diary records.
_selectedDate = null;
...
// If the selected date has diary records, make that record date
// visible in the sparse timeline. Dates with no records are not
// displayed in the timeline.
WidgetsBinding.instance.addPostFrameCallback((_) {
if (!_timelineController.hasClients) return;

final recordDates = _recordDatesWithCounts().keys.toList()
..sort((a, b) => b.compareTo(a));

final selectedIndex = recordDates.indexWhere(
(date) => _sameDay(date, pickedDate),
);
...
// With reverse:true, index 0 is the newest/right-most record.
final targetOffset =
...
Map<DateTime, int> _recordDatesWithCounts() {
...
// Only show dates that actually contain at least one diary record.
final dateCounts = _recordDatesWithCounts();

final dates = dateCounts.keys.toList()
..sort(
(a, b) => b.compareTo(a),
);
...
Widget buildTimelineEntry(int index) {
...
count: count,
selected: selected,
showLeftLine: index != dates.length - 1,
...
// Tap the selected date again to clear the filter.
_selectedDate = null;
} else {
_selectedDate = date;
}
...
final timelineWidth = dates.length * 66.0;

// When there are only a few record dates, keep the whole
// timeline visually centred in the page.
if (timelineWidth <= constraints.maxWidth) {
return Center(
child: Row(
mainAxisSize: MainAxisSize.min,
...
dates.length,
(visualIndex) {
// Keep the ORIGINAL timeline direction:
// older dates on the left, newest date on the right.
final index =
dates.length - 1 - visualIndex;

return buildTimelineEntry(index);
},
),
),
);
...
// When there are many record dates, keep horizontal scrolling.
