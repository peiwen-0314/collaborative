...
// Pushed from ProfilePage's "Travel Preferences" tile. This is the
// person's INTERESTS (what recommendations get personalised around),
// not the ride/transport routing settings TravelPreferencesSheet
// covers - RideHomePage's own tune icon still opens that one.
//
// Reuses the same PersonalizationController + interest-card grid
// InterestSelectionPage uses for first-time onboarding, just as a
// page the person can come back to and edit any time: it loads their
// already-saved interests (PersonalizationController.loadCategories()
// seeds selectedInterestIds from Firestore) instead of starting empty,
// and pops back to Profile on save instead of navigating to HomePage.
...
class _EditInterestsPageState extends State<EditInterestsPage> {
  ...
  Future<void> _save() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Travel preferences updated')),
    );
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'Choose what you enjoy and we will personalize '
                    'attraction recommendations for you.',
                    style: TextStyle(
                      fontSize: 13,
                      height: 1.45,
                      color: secondaryText,
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          onPressed: _controller.isSavingInterests ? null : _save,
          ...
          child: _controller.isSavingInterests
            ...
            child: CircularProgressIndicator(
              ...
              : Text(
            hasSelection
                ? 'Save  •  ${_controller.selectedInterestIds.length} selected'
                : 'Save',
            style: const TextStyle(
              fontSize: 14,
