...
import 'review_photo_service.dart';
...
class AttractionReviewsService extends ChangeNotifier {
  ...
  final Map<String, List<AttractionReview>>
  ...
  List<AttractionReview> reviewsFor(
      String attractionId,
      ) {
    return List.unmodifiable(
      _reviewsByAttraction[attractionId] ??
  ...
  int reviewCountFor(
    ...
    return _reviewsByAttraction[attractionId]
        ?.length ??
        0;
  }

  double averageRatingFor(
      String attractionId,
      ) {
    final reviews =
    _reviewsByAttraction[attractionId];
  ...
  void _listenToReviews() {
    ...
    _subscription = _firestore
        ...
        .listen(
          (snapshot) {
        final previousReviews =
        <String, AttractionReview>{};

        for (final reviews
        in _reviewsByAttraction.values) {
          for (final review in reviews) {
            previousReviews[review.id] =
                review;
          }
        }

        final grouped =
        <String, List<_DatedReview>>{};
        ...
        final activeReviewIds =
        _reviewsByAttraction.values
            .expand(
              (reviews) => reviews,
        )
            .map(
        ...
        final removedReviewIds =
        _commentSubscriptions.keys
            .where(
              (reviewId) =>
          !activeReviewIds
              .contains(
        ...
        for (final entry
        in _reviewsByAttraction.entries) {
          for (final review
          in entry.value) {
            _listenToComments(
              entry.key,
  ...
  void _listenToComments(
    ...
    if (_commentSubscriptions
        .containsKey(reviewId)) {
      return;
    }

    _commentSubscriptions[reviewId] =
        _firestore
            .collection(
          'attraction_reviews',
        )
  ...
  Future<void> deleteReview(
    ...
    await ReviewPhotoService.instance
        .deletePhotos(
      photoUrls,
    );
